...
class pig_meta_box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'admin_enqueue_scripts', array( &$this, 'js_css' ) );
	}
	...
	function show() {
		...
		wp_nonce_field( basename( __FILE__ ), 'cgc_meta_box_nonce' );
	...
	function save( $post_id ) {
		...
		if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )      // check autosave
			...
			|| ( !check_admin_referer( basename( __FILE__ ), 'cgc_meta_box_nonce' ) )  // verify nonce
		...
		foreach ( $this->_fields as $field ) {
			...
			// validate meta value
			if ( class_exists( 'cgc_meta_box_Validate' ) && method_exists( 'cgc_meta_box_Validate', $field['validate_func'] ) ) {
				$new = call_user_func( array( 'cgc_meta_box_Validate', $field['validate_func'] ), $new );
			}
