...
class pig_meta_box {
	...
	function js_css() {
		...
		$base_url = WP_PLUGIN_URL . '/' . str_replace(basename( __FILE__), "" ,plugin_basename(__FILE__));		// if this file is placed under a plugin
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'pig_meta_box_nonce');
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='rw-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='20' />";
	...
	function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='rw-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='rw-upload-single-image' value='Upload'/>";
		$this->show_field_end($field, $meta);
	}
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'pig_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('pig_meta_box_Validate') && method_exists('pig_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('pig_meta_box_Validate', $field['validate_func']), $new);
			}
